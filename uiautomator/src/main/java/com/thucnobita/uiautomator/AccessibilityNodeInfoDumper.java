...
class AccessibilityNodeInfoDumper {
    ...
    public static void dumpWindowHierarchy(Instrumentation instrumentation, UiDevice device, OutputStream out) throws IOException {
        ...
        AccessibilityNodeInfo[] arr$ = getWindowRoots(instrumentation, device); // device.getWindowRoots();
        int len$ = arr$.length;
    ...
    private static AccessibilityNodeInfo[] getWindowRoots(Instrumentation instrumentation, UiDevice device) {
        device.waitForIdle();
